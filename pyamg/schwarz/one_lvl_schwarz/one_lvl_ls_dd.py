...
def one_level_ls_dd_solver(
    ...
    *,
    ...
    stats = LsddLevelStats(level=0, n_fine=A_csr.shape[0])
    ...
    # One-level setup does not solve local GEPs, so set diagnostics placeholders.
    level.eigs.threshold = float("nan")

    _lsdd_finalize_level_stats(
        stats=stats,
        level=level,
        eigvals_kept=[],
        n_coarse=int(level.n_aggs),
    )
